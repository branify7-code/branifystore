...
export const PortfolioPage: React.FC<PortfolioPageProps> = ({ navigate, selectedSlug }) => {
  ...
  return (
    <div className="min-h-screen bg-[#05060B] text-white py-12 px-4 sm:px-6 lg:px-8 space-y-12 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[700px] h-[350px] bg-[#C9A45C]/10 rounded-full blur-[140px] pointer-events-none"></div>
      ...
      {selectedItem ? (
        ...
        <div className="max-w-5xl mx-auto space-y-10 relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-3 border-b border-white/[0.08] text-xs font-semibold text-zinc-400">
            ...
            <button
              onClick={() => navigate('/portfolio')}
              className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#E2C27B] hover:text-white transition-colors cursor-pointer"
            >
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center gap-3">
              <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[#E2C27B]/10 border border-[#E2C27B]/30 text-[#E2C27B] text-[10px] font-extrabold uppercase tracking-widest">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-zinc-400">
              ...
              <div className="text-zinc-400">
                Crafted & Engineered by <span className="text-[#E2C27B] font-bold">BRANIFY</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-zinc-300">
                <Monitor className="w-4 h-4 text-[#E2C27B]" />
            ...
            <div className="rounded-3xl overflow-hidden border border-white/[0.15] shadow-2xl bg-[#080B14]">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {selectedItem.liveUrl && (
                    <a
                      ...
                      className="px-3 py-1.5 bg-[#E2C27B]/15 text-[#E2C27B] hover:bg-[#E2C27B]/25 border border-[#E2C27B]/30 rounded-full text-[10px] font-extrabold uppercase tracking-wider transition-all flex items-center gap-1.5"
                    >
              ...
              <div className="bg-[#0D121F] px-4 py-2.5 border-t border-white/[0.08] flex flex-wrap items-center justify-between text-[11px] text-zinc-400 gap-2">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setIsFullscreenModalOpen(true)}
                    className="text-[#E2C27B] hover:underline font-bold flex items-center gap-1 cursor-pointer"
                  >
          ...
          <div className="bg-[#080B14]/90 border border-white/[0.1] rounded-3xl p-8 sm:p-10 space-y-6 shadow-xl backdrop-blur-xl">
            <div className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#E2C27B]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4 bg-[#080B14]/80 p-8 rounded-3xl border border-white/[0.1] shadow-xl backdrop-blur-xl">
              <div className="flex items-center gap-2 text-[#E2C27B] text-xs font-black uppercase tracking-wider">
          ...
          {selectedItem.keyFeatures && selectedItem.keyFeatures.length > 0 && (
            <div className="bg-[#080B14]/90 border border-white/[0.1] rounded-3xl p-8 sm:p-10 space-y-6 shadow-xl backdrop-blur-xl">
              <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2">
                <Cpu className="w-4 h-4 text-[#E2C27B]" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {selectedItem.keyFeatures.map((feat, idx) => (
                  <div key={idx} className="flex items-start gap-3 p-4 bg-white/[0.02] border border-white/[0.06] rounded-2xl">
                    <CheckCircle2 className="w-4 h-4 text-[#E2C27B] shrink-0 mt-0.5" />
          ...
          {selectedItem.results && selectedItem.results.length > 0 && (
            <div className="space-y-6 bg-[#080B14] p-8 sm:p-10 rounded-3xl border border-white/[0.1] shadow-xl">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-black text-[#E2C27B] uppercase tracking-wider flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {selectedItem.results.map((res, idx) => (
                  <div key={idx} className="p-6 bg-white/[0.03] border border-white/[0.08] hover:border-[#E2C27B]/40 rounded-2xl text-center space-y-2 transition-all">
                    <div className="w-8 h-8 rounded-full bg-[#E2C27B]/10 text-[#E2C27B] flex items-center justify-center mx-auto text-xs font-black">
                      0{idx + 1}
                    </div>
          ...
          <div className="bg-gradient-to-r from-blue-950/40 via-indigo-950/30 to-purple-950/20 border border-[#E2C27B]/30 rounded-3xl p-8 sm:p-10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
            <div className="space-y-2 text-center md:text-left">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#E2C27B]">
          ...
          {relatedProjects.length > 0 && (
            <div className="space-y-6 pt-6 border-t border-white/10">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => navigate('/portfolio')}
                  className="text-xs font-bold text-[#E2C27B] hover:underline flex items-center gap-1 cursor-pointer"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {relatedProjects.map((p) => (
                  <div
                    ...
                    className="bg-[#080B14] hover:bg-[#0D121F] border border-white/10 hover:border-[#E2C27B]/40 rounded-3xl overflow-hidden transition-all duration-300 group cursor-pointer shadow-xl flex flex-col justify-between"
                  >
                    <div className="h-44 overflow-hidden relative bg-[#05060B] p-2 flex items-center justify-center">
                      ...
                      />
                      <div className="absolute top-2.5 left-2.5 px-2.5 py-0.5 bg-black/80 backdrop-blur-md border border-white/10 rounded-full text-[9px] font-extrabold text-[#E2C27B] uppercase tracking-wider">
                        {p.category}
                      </div>
                    </div>
                    <div className="p-4 space-y-2 flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <h4 className="text-xs font-black text-white group-hover:text-[#E2C27B] transition-colors uppercase tracking-tight line-clamp-2">
                          {p.title}
                        </h4>
                      </div>
                      <div className="pt-2 border-t border-white/5 flex items-center justify-between text-[10px] font-bold text-zinc-400 group-hover:text-white">
                        ...
                        <ArrowRight className="w-3 h-3 text-zinc-500 group-hover:text-[#E2C27B] group-hover:translate-x-1 transition-all" />
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 space-y-12 relative z-10">
          ...
          <div className="text-center space-y-4 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-white/[0.05] border border-white/[0.12] text-[#E2C27B] text-[10px] font-extrabold uppercase tracking-widest backdrop-blur-md">
            ...
            <h1 className="text-4xl sm:text-6xl font-black text-white uppercase tracking-tight">
              Selected Work & <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E2C27B] via-[#C9A45C] to-[#E2C27B]">Websites</span>
            </h1>
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-xs font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
                  selectedCategory.toLowerCase() === cat.toLowerCase()
                    ? 'btn-gradient-primary text-white shadow-lg shadow-[#E2C27B]/20'
          ...
          {/* Portfolio Grid — shared PortfolioCard (matches homepage Selected Work) */}
          ...
          <div className="bg-[#080B14]/90 border border-white/[0.1] rounded-3xl p-8 sm:p-12 text-center space-y-6 max-w-4xl mx-auto shadow-2xl backdrop-blur-xl">
            ...
            <h2 className="text-2xl sm:text-4xl font-black text-white uppercase tracking-tight">
              Ready to Build Your Next <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E2C27B] to-[#C9A45C]">High-Performance Website</span>?
            </h2>
