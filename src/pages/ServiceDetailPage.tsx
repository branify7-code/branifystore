...
export const ServiceDetailPage: React.FC<ServiceDetailPageProps> = ({ slug, navigate }) => {
  ...
  // Clean, search-optimized meta title & description for each service
  const seoServiceTitle = service.slug === 'website-development'
    ? 'Website Development Services | BRANIFY'
    : `${service.name} Services | BRANIFY`;
  const seoServiceDesc = service.slug === 'website-development'
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
      <SEOHead
        title={seoServiceTitle}
        description={seoServiceDesc}
        ...
        faqs={service.faqs ? service.faqs.map(f => ({ question: f.question, answer: f.answer })) : undefined}
        serviceSchema={{
          name: service.name,
          description: service.fullDescription || service.shortDescription,
          serviceType: service.category === 'web' ? 'Web Development & Design' : service.category === 'design' ? 'Brand & Graphic Design' : 'Digital Growth & AI Services',
          providerName: 'BRANIFY'
        }}
      />
