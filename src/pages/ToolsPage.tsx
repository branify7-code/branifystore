...
export const ToolsPage: React.FC<ToolsPageProps> = ({ navigate, selectedToolSlug }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
      {activeTool ? (
        <SEOHead
          title={`${activeTool.name} — Free Online Tool | BRANIFY`}
          description={`${activeTool.description} Free online browser utility with instant processing and zero privacy tracking.`}
          canonicalPath={`/tools/${activeTool.slug}`}
          breadcrumbs={[
            { name: 'Home', url: '/' },
            { name: 'Free Online Tools', url: '/tools' },
            { name: activeTool.category, url: `/tools?category=${encodeURIComponent(activeTool.category)}` },
            { name: activeTool.name, url: `/tools/${activeTool.slug}` }
          ]}
          softwareSchema={{
        ...
        <SEOHead
          ...
          description="Use 100+ free online browser tools for productivity, development, text, images and everyday tasks. No signup required."
          canonicalPath="/tools"
          breadcrumbs={[
            { name: 'Home', url: '/' },
            { name: 'Free Online Tools', url: '/tools' }
