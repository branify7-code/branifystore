...
export const FreeTemplateDetailPage: React.FC<FreeTemplateDetailPageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 pb-20">
      <SEOHead
        title={template.seoTitle || `${template.title} — Free Download | BRANIFY`}
        description={template.shortDescription || `Download ${template.title} for free in ${template.fileFormat} format. Instant download with commercial usage permitted.`}
        canonicalPath={`/free-templates/${template.slug}`}
        ogImage={template.previewImage}
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Free Templates', url: '/free-templates' },
          { name: template.category, url: `/free-templates/${template.categorySlug}` },
          { name: template.title, url: `/free-templates/${template.slug}` }
        ]}
        softwareSchema={{
          name: template.title,
